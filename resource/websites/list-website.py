...
import urllib3
import sys
from bs4 import BeautifulSoup  # ✅ Required for summarizing HTML

urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)

# === CyberPanel Configuration ===
CYBERPANEL_URL = "https://192.168.42.80:8090"
WEBSITE_API_URL = f"{CYBERPANEL_URL}/websites/fetchWebsitesList"  # UPDATED HERE
...
def load_or_auth_admin_session():
    ...
    if session:
        ...
        try:
            # NEW payload here
            test = session.post(
                WEBSITE_API_URL, headers=headers, json={"page": 1, "recordsToShow": 10}, verify=False
            )
...
def list_websites(session):
    print("[*] Fetching website list...")
    headers = get_headers(session)
    payload = {"page": 1, "recordsToShow": 10}  # UPDATED PAYLOAD HERE
